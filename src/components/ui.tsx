  ...
  return (
    <span className={`inline-flex items-center gap-0.5 ${className}`} aria-label={`${value} out of 5 stars`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <LuStar
          key={i}
          className={`h-4 w-4 ${i < value ? "fill-amber-400 text-amber-400" : "text-line"}`}
        />
      ))}
    </span>
  );
...
export function Eyebrow({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full bg-sky px-3.5 py-1.5 text-xs font-bold uppercase tracking-[0.14em] text-blue-700 ${className}`}
    >
      {children}
    </span>
  );
...
export function SectionHeading({
  ...
  return (
    <div className={`${center ? "mx-auto max-w-2xl text-center" : "max-w-2xl"}`}>
      ...
      {intro && (
        <p className={`mt-4 text-base leading-relaxed ${light ? "text-sky/80" : "text-slatey"}`}>
          {intro}
        </p>
      )}
    </div>
