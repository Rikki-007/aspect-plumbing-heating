...
export function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 shadow-soft backdrop-blur" : "bg-white/80 backdrop-blur"}`}>
          ...
          {nav.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${active ? "bg-sky text-blue-700" : "text-navy hover:bg-mist"}`}>
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="border-t border-line bg-white md:hidden">
          <nav className="container-x flex flex-col py-3">
            {nav.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={`rounded-lg px-4 py-3 text-base font-semibold ${active ? "bg-sky text-blue-700" : "text-navy"}`}>
                  {item.label}
                </Link>
              );
            })}
