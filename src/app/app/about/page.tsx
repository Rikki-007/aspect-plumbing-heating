...
import { LuPhone, LuShieldCheck, LuBadgeCheck, LuClock, LuMessageSquare } from "@/components/icons";
...
const values = [
  { icon: LuShieldCheck, title: "Trust first", text: "One familiar face on every job. The same tradesman you spoke to is the one who turns up and does the work." },
  { icon: LuBadgeCheck, title: "Fair pricing", text: "Clear prices agreed up front. No inflated call-out fees, no padding the bill once we're in the door." },
  { icon: LuClock, title: "There when it matters", text: "Plumbing emergencies don't wait for office hours — so we answer the phone day, night and weekends." },
  { icon: LuMessageSquare, title: "Straight talk", text: "We explain what's wrong in plain English, what your options are, and what we'd do if it were our own home." },
];

export default function AboutPage() {
  return (
    <>
      <PageHero crumb="About" eyebrow="Meet your plumber"
      ...
      <section className="bg-white py-16 sm:py-20">
        <div className="container-x grid items-center gap-12 lg:grid-cols-2">
          <Reveal><MediaTile label="Anthony — owner & lead plumber" icon="fire" aspect="aspect-[4/5]" className="shadow-card" /></Reveal>
          <div>
            <Reveal>
              <SectionHeading eyebrow="Our story" title="The face behind Aspect"
            ...
            <Reveal delay={0.15}>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <Button href={telHref} variant="flame" external><LuPhone className="h-4 w-4" /> Call {site.phoneDisplay}</Button>
      ...
      <section className="bg-mist py-16 sm:py-20">
        <div className="container-x">
          <Reveal>
            <SectionHeading center eyebrow="What we stand for" title="How we work"
      ...
      <section className="bg-white py-16 sm:py-20">
        <div className="container-x grid items-center gap-12 lg:grid-cols-2">
          <div>
            <Reveal><SectionHeading eyebrow="Our promise to you" title="What you can always expect" /></Reveal>
            <Reveal delay={0.1}>
              <ul className="mt-8 space-y-4">
                ...
                ].map((p) => (<CheckItem key={p}>{p}</CheckItem>))}
              </ul>
            </Reveal>
          </div>
          <Reveal delay={0.1}><MediaTile label="Quality workmanship" icon="radiator" aspect="aspect-[4/3]" className="shadow-card" /></Reveal>
        </div>
      </section>
        ...
        <div className="container-x">
          <Reveal>
            <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
              ...
              <div className="shrink-0"><RatingBadge /></div>
            </div>
          </Reveal>
          <div className="mt-12"><Reviews /></div>
        </div>
      </section>
