...
export default function ServicesPage() {
  return (
    <>
      <PageHero crumb="Services" eyebrow="What we do"
              ...
              <StaggerItem key={s.slug}>
                <article id={s.slug}
                  ...
                  <div className="flex items-center gap-4 lg:flex-col lg:items-start">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-slatey/60">0{i + 1}</span>
                  </div>
                  <div>
                    <h2 className="text-2xl font-bold text-navy">{s.title}</h2>
                    <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slatey">{s.blurb}</p>
                    <ul className="mt-5 grid gap-2.5 sm:grid-cols-2">
                      {s.points.map((p) => (<CheckItem key={p}>{p}</CheckItem>))}
                    </ul>
                  </div>
          ...
          <Reveal>
            <div className="mt-12 rounded-3xl bg-mist p-8 sm:p-10">
              <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                ...
                <Button href="/contact" variant="navy" className="shrink-0">Request a quote <LuArrowRight className="h-4 w-4" /></Button>
              </div>
            </div>
          </Reveal>
        </div>
